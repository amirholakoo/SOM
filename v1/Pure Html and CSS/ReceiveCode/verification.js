...
// Copy code and continue to step 2
function copyCodeAndContinue() {
    // First copy the code
    navigator.clipboard.writeText(verificationCode).then(() => {
        showMessage('کد کپی شد و به مرحله بعد منتقل شدید', 'success');
        // Then go to step 2
        setTimeout(() => {
            goToStep2();
        }, 500);
        ...
        showMessage('خطا در کپی کردن کد', 'error');
        // Still go to step 2 even if copy fails
        setTimeout(() => {
            goToStep2();
        }, 500);
    });
